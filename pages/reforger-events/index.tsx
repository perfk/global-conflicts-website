        ...
        <>
            ...
            <div className="flex flex-col max-w-screen-xl px-2 mx-auto mb-10">
                <div className="mx-4 mt-10 prose lg:prose-xl text-gray-900 dark:text-gray-100" style={{ maxWidth: "none" }}>
                    <h1 className="text-gray-900 dark:text-white">Reforger Events</h1>
                            ...
                            <Tab.Panel>
                                <div className="mx-1 my-10 space-y-10 md:mx-12">
                                    {upcomingEvents.map((event) => (
                                        <Link key={event._id} href={`/reforger-events/${event.slug}`} passHref legacyBehavior>
                                            <a>
                            ...
                            <Tab.Panel>
                                <div className="mx-1 my-10 space-y-10 md:mx-12">
                                    {pastEvents.map((event) => (
                                        <Link key={event._id.toString()} href={`/reforger-events/${event.slug}`} passHref legacyBehavior>
                                            <a>
                                                <EventCard contentHeight="auto" event={event}></EventCard>
                                            </a>
                                        </Link>
...
export const getServerSideProps: GetServerSideProps = async () => {
    const pastEvents = await (await MyMongo).db("prod").collection("reforger_events")
        .find(
            {
    ...
    const upcomingEvents = await (await MyMongo).db("prod").collection("reforger_events")
        .find(
            {
                "closeReason.value": {
