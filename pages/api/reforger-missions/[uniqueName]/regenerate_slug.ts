...
import { findReforgerMissionBySlug, makeSafeName } from "../../../../lib/missionsHelpers";
...
apiRoute.post(async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    const baseSafeName = makeSafeName(mission.name);
    let newSlug = baseSafeName;

    // Check for collisions
    const existing = await db.collection("reforger_missions").findOne({ 
    ...
    // 1. Update the mission itself
    await db.collection("reforger_missions").updateOne(
        { _id: mission._id },
        { 
            $set: { uniqueName: newSlug },
            $addToSet: { previousSlugs: currentSlug }
        }
    );

    // 2. Update foreign keys if they relied on the old slug instead of a GUID
    await db.collection("reforger_mission_metadata").updateMany(
        { missionId: currentSlug },
        { $set: { missionId: newSlug } }
    );

    // 3. Update sessions
    await db.collection("server_sessions").updateMany(
        { missionUniqueName: currentSlug },
        { $set: { missionUniqueName: newSlug } }
    );
