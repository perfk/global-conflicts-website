...
const EventCard: React.FC<EventCardProps> = ({
	...
	useEffect(() => {
		try {
			const isHour12 = new Intl.DateTimeFormat(undefined, { hour: 'numeric' }).resolvedOptions().hour12;
			setHour12(isHour12 ?? true);
		} catch (e) {
			console.error("Error detecting time format", e);
		}
	}, []);
	...
	function getCurrentSignUps() {
		if (overrideSignupCount !== undefined) return overrideSignupCount;
					...
					<div className="flex flex-row flex-wrap">
						...
						<div className="flex flex-row flex-wrap items-end justify-start ml-auto">
							<div className="mr-10 text-white bg-transparent">
								...
								<div>{moment(event.when).format(hour12 ? "lll" : "DD MMM YYYY, HH:mm")}</div>
							</div>
